...
```bash
server$ sam build --use-container
```
...
```bash
server$ pip install -r tests/requirements.txt --user
# unit test
server$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
server$ AWS_SAM_STACK_NAME="server" python -m pytest tests/integration -v
```
